...
const Cubes = ({
  ...
  useEffect(() => {
    ...
    const speed = 0.02;
    // Perf: only re-tilt when the simulated position has moved meaningfully,
    // instead of creating fresh tweens for every cube on every frame.
    let lastX = Infinity;
    let lastY = Infinity;
    const loop = () => {
      if (!userActiveRef.current) {
        ...
        if (Math.abs(pos.x - lastX) > 0.02 || Math.abs(pos.y - lastY) > 0.02) {
          tiltAt(pos.y, pos.x);
          lastX = pos.x;
          lastY = pos.y;
        }

...
export default function Carousel({
  ...
  useImperativeHandle(ref, () => ({
    prev: () => setPosition((p) => Math.max(0, p - 1)),
    next: () => setPosition((p) => Math.min(itemsForRender.length - 1, p + 1)),
  ...
  const activeIndex =
    items.length === 0 ? 0 : loop ? (position - 1 + items.length) % items.length : Math.min(Math.max(position, 0), items.length - 1);
